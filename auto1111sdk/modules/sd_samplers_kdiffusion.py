...
import inspect
import sys
from .k_diffusion.k_diffusion import sampling
from . import sd_samplers_common, sd_samplers_extra, sd_samplers_cfg_denoiser
from .script_callbacks import ExtraNoiseParams, extra_noise_callback

from .k_diffusion import k_diffusion as kd
...
samplers_data_k_diffusion = [
    sd_samplers_common.SamplerData(
        label,
        lambda model, tqdm_val, funcname=funcname: KDiffusionSampler(funcname, model, tqdm_val),
        aliases,
        options
    )
    for label, funcname, aliases, options in samplers_k_diffusion
    if callable(funcname) or hasattr(sampling, funcname)
]
...
k_diffusion_scheduler = {
    'Automatic': None,
    'karras': sampling.get_sigmas_karras,
    'exponential': sampling.get_sigmas_exponential,
    'polyexponential': sampling.get_sigmas_polyexponential
}
...
class CFGDenoiserKDiffusion(sd_samplers_cfg_denoiser.CFGDenoiser):
    ...
    def inner_model(self):
        if self.model_wrap is None:
            denoiser = kd.external.CompVisVDenoiser if self.sd_model.parameterization == "v" else kd.external.CompVisDenoiser  #shared.sd_model.parameterization == "v" else k_diffusion.k_diffusion.external.CompVisDenoiser
            self.model_wrap = denoiser(self.sd_model, quantize=shared.opts.enable_quantization)

        return self.model_wrap


class KDiffusionSampler(sd_samplers_common.Sampler):
    def __init__(self, funcname, sd_model, tqdm_val, options=None):
        super().__init__(funcname, sd_model, tqdm_val)
        ...
        self.model_wrap_cfg = CFGDenoiserKDiffusion(sd_model)
        self.model_wrap_cfg.sd_model = sd_model 
    ...
    def get_sigmas(self, p, steps):
        ...
        if p.sampler_noise_scheduler_override:
        ...
        elif shared.opts.k_sched_type != "Automatic":
            ...
            sigmas_func = k_diffusion_scheduler[shared.opts.k_sched_type]
            p.extra_generation_params["Schedule type"] = opts.k_sched_type

            if shared.opts.sigma_min != m_sigma_min and shared.opts.sigma_min != 0:
                sigmas_kwargs['sigma_min'] = shared.opts.sigma_min
                p.extra_generation_params["Schedule min sigma"] = opts.sigma_min
            if shared.opts.sigma_max != m_sigma_max and shared.opts.sigma_max != 0:
                sigmas_kwargs['sigma_max'] = shared.opts.sigma_max
                p.extra_generation_params["Schedule max sigma"] = opts.sigma_max

            default_rho = 1. if shared.opts.k_sched_type == "polyexponential" else 7.

            if shared.opts.k_sched_type != 'exponential' and shared.opts.rho != 0 and shared.opts.rho != default_rho:
            ...
            sigmas = sampling.get_sigmas_karras(n=steps, sigma_min=sigma_min, sigma_max=sigma_max, device=shared.device)
        elif self.config is not None and self.config.options.get('scheduler', None) == 'exponential':
            m_sigma_min, m_sigma_max = (self.model_wrap.sigmas[0].item(), self.model_wrap.sigmas[-1].item())
            sigmas = sampling.get_sigmas_exponential(n=steps, sigma_min=m_sigma_min, sigma_max=m_sigma_max, device=shared.device)
        else:
