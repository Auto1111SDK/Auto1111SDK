...
from . import script_callbacks, shared, devices
...
def get_unet_option(sd_model, option=None):
    ...
    if option == "Automatic":
        name = sd_model.sd_checkpoint_info.model_name
...
def apply_unet(sd_model, option=None):
    global current_unet_option
    global current_unet

    new_option = get_unet_option(sd_model, option)
    ...
    if current_unet_option is None:
        ...
        if not sd_model.lowvram:
            sd_model.model.diffusion_model.to(devices.device)

        return

    sd_model.model.diffusion_model.to(devices.cpu)
    devices.torch_gc()
